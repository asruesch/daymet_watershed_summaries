...
outputFolder="J:/daymet/climSummaries/modelClimateData2"
...
# datfiles="C:/Users/menuzd/Desktop/lags/rawData"
# outputFolder="C:/Users/menuzd/Desktop/lags/output2"
# daysfile="Z:/Diebel/DGEL/transfer/daymet/output/daysLookup.csv"
# logFile="C:/Users/menuzd/Desktop/lags/lagNA"
# naFiles=c()
...
# lags = rbind(c(1, 3), c(4, 6), c(7, 9), c(10, 12), c(13, 24), c(25, 48))
# sStart=1
# sEnd=length(fileList)


###################################################################################
calcClimate=function(m, firstM, period){
if (period != "wy") {# if we are not calculating water year data
	...
	waterYearDat2=subset(waterYearDat, waterYearDat$waterYear  %in% c(1981:2011))
	...
	periodLag=subset(waterYearDat, waterYearDat$month==firstM)
	periodLag$year=periodLag$year+1
	periodLag=subset(periodLag, periodLag$year !=2012)
}
...
for (l in unique(lagDat$lagStart)){
	...
	assign(paste("plag", l, sep=""), newDat)
	}
plag1=subset(lagDat, lagDat$lagStart==1)
lagCombo=data.frame(plag1, plag4, plag7, plag10, plag13, plag25)
lagPeffFinal=lagCombo[,c(1:2, 4:9)]
colnames(lagPeffFinal)[3]="plag1"
